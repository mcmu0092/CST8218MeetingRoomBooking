...
<body >
	...
	<h2 class="col-sm-offset-1" ><%=resource.getString("booking.label.header") %></h2>
		...
		if(listSize != 0){
			for(i = 0; i < listSize; i++){%>
				<div  class="form-horizontal">
					...
					<div class="form-group">
						...
						<label class="control-label  "><%=resource.getString("booking.label.capacity") %></label>
						<label class="control-label "><%=roomList.get(i).getCapacity()%></label>
					</div>
					
					<div class="form-group">
						<label class="control-label col-sm-2"> <%=resource.getString("booking.label.roominfo") %> </label>
						<label class="control-label "><%=roomList.get(i).getInfo()%></label>
					</div>
					
					<div class="form-group">
						<div class="col-sm-offset-2 col-sm-4">
							<button class="btn btn-default" id="showBookingModal" data-toggle="modal" data-target="#roomBooking" data-building_num="<%=roomList.get(i).getBuildingID() %>" data-room_num="<%=roomList.get(i).getRoomID() %>"><%=resource.getString("booking.button.book") %></button>
						</div>
					</div>
			...
			if(rowOffSet > 1 ){
				out.print("<a href=\"Booking.jsp?rowOffSet="+(rowOffSet-limit)+"\" >"+ resource.getString("booking.link.previous")+"</a>");
			}
			
			if((rowOffSet+i) != maxRows){
				out.print("<a href=\"Booking.jsp?rowOffSet="+(rowOffSet+limit)+"\" >"+ resource.getString("booking.link.next")+"</a>");
			}
